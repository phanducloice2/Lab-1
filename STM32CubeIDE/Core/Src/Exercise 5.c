...
void run_exercise5(){
	if (count>7){led_red1();display7SEG(count-6);led_green2();count--;}
	else if (count>5){led_red1();display7SEG(count-6);led_yellow2();count--;}
	else if (count>2){led_green1();display7SEG(count-3);led_red2();count--;}
	else if (count>0){led_yellow1();led_red2();display7SEG(count-1);count--;}
	if (count==0) count=10;
}
